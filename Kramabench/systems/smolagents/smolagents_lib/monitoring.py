...
class AgentLogger:
    def __init__(self, level: LogLevel = LogLevel.INFO, console: Console | None = None, log_file: str | None = None):
        self.level = level
        ...
        if self.log_file is not None:
            ...
            # Set up a console for logging to file
            self.file_console_stream = StringIO()
            self.file_console = Console(file=self.file_console_stream, force_terminal=False)
        ...
        if level <= self.level:
            ...
            if self.log_file:
                ...
                with open(self.log_file, "a") as f:
                    f.write(self.file_console_stream.getvalue())
                self.file_console_stream.truncate(0)
                self.file_console_stream.seek(0)
