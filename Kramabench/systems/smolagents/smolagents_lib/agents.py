...
class MultiStepAgent(ABC):
    ...
    def __init__(
        ...
        else: self.planner_model = planner_model
...
{str(additional_args)}."""
    ...
    def _generate_planning_step(
        ...
        if is_first_step:
            ...
            if self.stream_outputs and hasattr(self.planner_model, "generate_stream"):
                plan_message_content = ""
                output_stream = self.planner_model.generate_stream(input_messages, stop_sequences=["<end_plan>"])  # type: ignore
                ...
                plan_message = self.planner_model.generate(input_messages, stop_sequences=["<end_plan>"])
            ...
            if self.stream_outputs and hasattr(self.planner_model, "generate_stream"):
                ...
                input_tokens, output_tokens = 0, 0
                with Live("", console=self.logger.console, vertical_overflow="visible") as live:
                    for event in self.planner_model.generate_stream(
                        input_messages,
