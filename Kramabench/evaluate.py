...
def _flatten_measure(metric, value):
    if isinstance(value, dict):
        for submetric, subvalue in value.items():
            yield f"{metric}.{submetric}", subvalue
        return
    yield metric, value
...
def aggregate_results(system_name, results_df):
    ...
    for (workload, metric), group in results_df.groupby(["workload", "metric"]):
        if metric in NON_AGGREGATE_METRICS or metric.startswith("trace_files."):
            continue
        group_dropped_na = group.dropna(subset=["value"]).copy()
        if metric == "success":
            group_dropped_na["value"] = group_dropped_na["value"].astype(bool).astype(int)

        if metric != "llm_code_eval":
            numeric_values = pd.to_numeric(group_dropped_na["value"], errors="coerce").dropna()
            if len(numeric_values) == 0:
                mean = 0.0 if metric in SCORE_METRICS else np.nan
                std = 0
                value_sum = 0.0
                value_support = 0
            else:
                mean = numeric_values.mean()
                std = numeric_values.std() if len(numeric_values) > 1 else 0
                value_sum = numeric_values.sum()
                value_support = len(numeric_values)
    ...
    for _, row in workload_results_df.iterrows():
        if row["metric"] in SCORE_METRICS:
            total_support += row["total_value_support"]
            total_score += row["value_support"] * row["value_mean"]
    print(f"Total score is: {total_score/total_support*100 if total_support else 0}")
        ...
        for task_result in evaluation_results:
            ...
            for metric, value in task_result.items():
                if metric in ["task_id", "code", "model_output", "subresponses"]:
                    continue
                for flat_metric, flat_value in _flatten_measure(metric, value):
                    parsed_value = flat_value
                    if isinstance(flat_value, (dict, list)): # LLM code eval results, handle list
                        parsed_value = f"\"{json.dumps(flat_value)}\""
